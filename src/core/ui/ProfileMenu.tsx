...
export function ProfileMenu() {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((o) => !o)}
        className="focus-ring flex min-h-10 items-center gap-2 rounded-full border border-line bg-control-bg py-1 pr-3 pl-1 transition hover:border-cyan/50 hover:bg-control-hover"
        aria-label="ユーザー設定"
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 z-50 mt-2 w-72 rounded-3xl border border-line bg-bg-panel p-4 shadow-[var(--lift-shadow)]">
            ...
            <div className="mb-4 flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <button
                  ...
                  className="focus-ring mt-1 flex items-center gap-1 rounded-md text-xs font-bold text-cyan hover:underline"
                >
                  <Pencil size={11} />
                  名前を変更
                </button>
              ...
              {AVATARS.map((a) => (
                <button
                  ...
                  className={cn(
                    ...
                    a === avatar && 'bg-cyan/15 ring-1 ring-cyan/60',
                  )}
                  aria-label={`アイコン ${a}`}
                >
                  {a}
            ...
            <button
              onClick={toggleMuted}
              className="focus-ring flex w-full items-center justify-between rounded-xl border border-line bg-control-bg px-3 py-2 text-sm font-bold text-fg transition hover:border-cyan/50 hover:bg-control-hover"
            >
              <span className="flex items-center gap-2">
                {muted ? <VolumeX size={15} /> : <Volume2 size={15} />}
                効果音
              </span>
              <span className={cn('text-xs font-bold', muted ? 'text-faint' : 'text-yellow')}>
                {muted ? 'OFF' : 'ON'}
              </span>
            </button>
          </div>
